...
def configVarsList = [
    ...
    "AUTODEPLOY_BRANCHES",    // ветка автодеплоя
...
properties([
    ...
    parameters([
       ...
       booleanParam(name: 'RUN_PRE_INSTALL_HOOK', defaultValue: false, description: 'Выполнить миграции перед отгрузкой'),
        ...
        stage('Checkout') {
            gitlabCommitStatus("checkout") {
                ...
                if (doDeploy) {
                    ...
                    def branchFolder = "ms-helm-values/${options.get("VALUES_PATH")}/${options.get("HELM_RELEASE")}"
                    def masterFolder = "ms-helm-values/${options.get("VALUES_PATH")}/${options.get("HELM_RELEASE")}"
        ...
        if (doDeploy) {
            ...
            stage('Deploy') {
                gitlabCommitStatus("deploy") {
                    ...
                    if (continueDeploy) {
                        def releaseName = "${options.get('HELM_RELEASE')}-${options.get("BRANCH_NAME")}".replace("_", "-")
                        def svcName = ""
                        def ingressHost = ""
                        ...
                        //if (params.DELETE_AFTER && !options.getAsList('NOT_AUTODELETE').contains(options.get("BRANCH_NAME"))) {
